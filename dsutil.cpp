...
IDirectSoundBuffer *DSFLoadSoundBuffer(IDirectSound *pDS, LPTSTR lpFilename)
{
	IDirectSoundBuffer *pDSB = NULL;
	DSBUFFERDESC dsBD = {0};
	HMMIO wavefile;
	...
	wavefile = mmioOpen( lpFilename, 0, MMIO_READ | MMIO_ALLOCBUF );
	if( wavefile != NULL )
	{
		...
		if( wavefmt.wFormatTag == WAVE_FORMAT_PCM )
		{
			...
			if( SUCCEEDED( IDirectSound_CreateSoundBuffer(pDS, &dsBD, &pDSB, NULL) ) )
			{
				if ( SUCCEEDED( IDirectSoundBuffer_Lock( pDSB, 0, child.cksize, &write1, &length1, &write2, &length2, 0) ) )
				{
					if ( write1 > 0 )
						mmioRead( wavefile, (char*)write1, length1 );
					if ( write2 > 0 )
						mmioRead( wavefile, (char*)write2, length2 );
					IDirectSoundBuffer_Unlock( pDSB, write1, length1, write2, length2 );
				}
			}
		}
...
BOOL DSParseWaveResource(void *pvRes, WAVEFORMATEX **ppWaveHeader, BYTE **ppbWaveData,DWORD *pcbWaveSize)
{
    ...
    while (pdw < pdwEnd)
    {
        ...
        case mmioFOURCC('f', 'm', 't', ' '):
            ...
            {
                if (dwLength < sizeof(WAVEFORMATEX))
                    goto exit;      // not a WAV
